...
impl Default for AudioFetchParams {
    fn default() -> Self {
        // Keep the cold-start request small enough that a temporarily slow CDN
        // response does not hold playback for several seconds. The decoder's
        // ring buffer has its own, independent 64 KiB minimum.
        let minimum_download_size = 32 * 1024;
        ...
        Self {
            ...
            download_timeout: Duration::from_secs(
                (minimum_download_size / minimum_throughput).max(8) as u64,
            ),
        }
    }
}
...
impl StreamLoaderController {
    ...
    /// Contiguous data already downloaded from the decoder's current read
    /// position. Callers can use this to start speculative work only after
    /// playback has enough headroom to survive it.
    pub fn available_bytes_from_read_position(&self) -> usize {
        match self.stream_shared {
            Some(ref shared) => {
                let read_position = shared.read_position();
                let download_status = shared
                ...
                download_status
                    .downloaded
                    .contained_length_from_value(read_position)
            }
            None => self.file_size,
        }
    }
    ...
    /// Request more contiguous data after the decoder's current position
    /// without blocking the player thread.
    pub fn fetch_next(&self, length: usize) {
        if let Some(ref shared) = self.stream_shared {
            self.fetch(Range::new(shared.read_position(), length));
        }
    }
...
impl AudioFileDownloadStatus {
    fn with_initial_request(length: usize) -> Self {
        let mut requested = RangeSet::new();
        requested.add_range(&Range::new(0, length));
        Self {
            requested,
    ...
    pub async fn open(
        ...
        for url in &urls {
            ...
            match streamer_result {
                Ok(r) => {
                    trace!(
                        "Initial CDN range 0+{minimum_download_size} headers after {} ms",
        ...
        let shared = Arc::new(AudioFileShared {
            ...
            bytes_per_second,
            cond: Condvar::new(),
            // The first request is already in flight. This matters once its
            // body is made visible incrementally: otherwise a decoder read
            // sees an incomplete range as missing and starts a duplicate
            // overlapping request.
            download_status: Mutex::new(AudioFileDownloadStatus::with_initial_request(
                initial_request.length,
            )),
...
mod tests {
    ...
    #[test]
    fn initial_cdn_range_is_already_marked_in_flight() {
        let status = AudioFileDownloadStatus::with_initial_request(65_536);
        assert!(status.requested.contains(0));
        assert!(status.requested.contains(65_535));
        assert!(!status.requested.contains(65_536));
        assert!(status.downloaded.is_empty());
    }

    #[test]
    fn startup_range_is_smaller_than_the_decoder_buffer() {
        let params = AudioFetchParams::default();
        assert_eq!(params.minimum_download_size, 32 * 1024);
        assert_eq!(params.download_timeout, Duration::from_secs(8));
    }
}
