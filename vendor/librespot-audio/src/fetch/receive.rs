...
async fn receive_data(
    ...
    let result: Result<_, Error> = 'responses: loop {
        ...
        // A CDN range is deliberately large enough to give Symphonia a useful
        // contiguous prefix for identifying and decoding the stream.
        // Waiting for `collect()` made that entire range invisible until the
        // slowest byte arrived, unnecessarily delaying cold starts and manual
        // skips. Publish each received HTTP frame instead: `AudioFileStreaming`
        // still reports only contiguous data, so decoder and range invariants
        // stay intact while it can make progress as soon as a prefix exists.
        while let Some(frame) = body.frame().await {
            let frame = match frame {
                Ok(frame) => frame,
                Err(e) => break 'responses Err(e.into()),
            };
            let Ok(data) = frame.into_data() else {
                continue;
            };
            publish_data(&file_data_tx, &mut offset, &mut actual_length, data)?;
        }
    };
...
mod tests {
    ...
    #[test]
    fn publishes_each_http_frame_at_its_contiguous_file_offset() {
        ...
        let ReceivedData::Data(first) = receiver.try_recv().unwrap() else {
            panic!("expected first data frame");
        };
        assert_eq!(first.offset, 32);
        assert_eq!(first.data.as_ref(), b"first");
        let ReceivedData::Data(second) = receiver.try_recv().unwrap() else {
            panic!("expected second data frame");
        };
        assert_eq!(second.offset, 37);
