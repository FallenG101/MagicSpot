...
const PRELOAD_NEXT_TRACK_BUFFER_DURATION_MS: usize = 5000;
...
impl PlayerTrackLoader {
    ...
    async fn load_remote_track(
        ...
        loop {
            ...
            // `Subfile::new` reset the source to Spotify's first playable
            // Ogg packet before the decoder was constructed. Seeking precisely
            // to zero again makes Symphonia build a remote seek path across the
            // file, delaying every cold start while it downloads distant ranges.
            // A non-zero resume still needs the accurate seek.
    ...
    fn poll(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<()> {
        ...
        loop {
            ...
            } = self.preload
            {
                ...
                match loader.as_mut().poll(cx) {
                    Poll::Ready(Ok(loaded_track)) => {
                        let cushion_bytes = preload_cushion_bytes(loaded_track.bytes_per_second);
                        loaded_track
                            .stream_loader_controller
                            .fetch_next(cushion_bytes);
            ...
            let allow_buffered_preload = matches!(&self.state, PlayerState::Playing { .. });
            if let PlayerState::Playing {
                ref track_id,
                play_request_id,
                bytes_per_second,
            ...
            } = self.state
            {
                ...
                let buffered_bytes = stream_loader_controller.available_bytes_from_read_position();
                let has_preload_headroom = allow_buffered_preload
                    && buffered_for_early_preload(buffered_bytes, bytes_per_second);

                if (!*suggested_to_preload_next_track)
                    && ((near_end && stream_loader_controller.range_to_end_available())
                        || has_preload_headroom)
                {
...
fn buffered_for_early_preload(buffered_bytes: usize, bytes_per_second: usize) -> bool {
    buffered_bytes >= preload_cushion_bytes(bytes_per_second)
}

fn preload_cushion_bytes(bytes_per_second: usize) -> usize {
    bytes_per_second.saturating_mul(PRELOAD_NEXT_TRACK_BUFFER_DURATION_MS) / 1000
}
...
mod tests {
    use super::{
        LoadError, PlayerEvent, PlayerTrackLoader, buffered_for_early_preload,
        preload_cushion_bytes,
    };
    ...
    #[test]
    fn early_preload_waits_for_five_seconds_of_downloaded_audio() {
        let bytes_per_second = 40_000;
        assert_eq!(preload_cushion_bytes(bytes_per_second), 200_000);
        assert!(!buffered_for_early_preload(199_999, bytes_per_second));
        assert!(buffered_for_early_preload(200_000, bytes_per_second));
    }
}
