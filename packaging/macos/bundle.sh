#!/bin/bash
# Build MagicSpot.app from a GUI binary, on a macOS machine.
#
...
cp "$binary" "$app/Contents/MacOS/magicspot"
chmod 755 "$app/Contents/MacOS/magicspot"
...
iconset="$(mktemp -d)/magicspot.iconset"
mkdir -p "$iconset"
...
iconutil -c icns "$iconset" -o "$app/Contents/Resources/magicspot.icns"
