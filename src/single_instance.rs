...
//! ends.
//!
//! On macOS and Windows, clients send one `MagicSpot:<verb>` line and receive
...
#[cfg(target_os = "linux")]
const MPRIS_NAME: &str = "org.mpris.MediaPlayer2.magicspot";
...
/// happens to hold the port is never mistaken for MagicSpot.
#[cfg(not(target_os = "linux"))]
const PREFIX: &str = "magicspot:";
#[cfg(not(target_os = "linux"))]
const OK_REPLY: &str = "magicspot:ok";
#[cfg(not(target_os = "linux"))]
const NOW_REPLY: &str = "magicspot:now ";
#[cfg(not(target_os = "linux"))]
const DEVICES_REPLY: &str = "magicspot:devices ";
...
fn send_to(port: u16, verb: &str) -> std::io::Result<Reply> {
    ...
    if line == OK_REPLY {
        ...
        Err(std::io::Error::new(
            ...
            "the port is held by something other than MagicSpot",
        ))
    }
}
...
pub fn acquire(waker: &crate::backend::Waker, link: Option<&str>) -> Outcome {
    ...
    let listener = match TcpListener::bind((Ipv4Addr::LOCALHOST, INSTANCE_PORT)) {
        ...
        Err(_) => {
            ...
            // MagicSpot. A link goes with the request; an instance from
            ...
            if link.is_some() && !opened {
                log::warn!("the running MagicSpot does not take links; asking it to show");
            }
            let answered = opened || send("show").is_ok_and(accepted);
            if answered {
                return Outcome::Surfaced;
            }
            log::warn!("port {INSTANCE_PORT} is busy but not with MagicSpot; running unguarded");
    ...
    let spawned = std::thread::Builder::new()
        .name("magicspot-instance".to_owned())
    ...
    match connection.request_name_with_flags(INSTANCE_NAME, RequestNameFlags::DoNotQueue.into()) {
        ...
        Ok(_) | Err(mpris_server::zbus::Error::NameTaken) => {
            if !raise_running_instance(&connection, link) {
                log::warn!(
                    "MagicSpot is already running but did not answer; not starting a second copy"
                );
            }
            Outcome::Surfaced
        }
...
fn open_in_running_instance(
    ...
    let spawned = std::thread::Builder::new()
        .name("magicspot-open-link".to_owned())
        .spawn(move || {
...
fn raise_running_instance(
    ...
    for attempt in 0..10 {
        ...
        if raised.is_ok() {
            if link.is_some() {
                log::warn!("the running MagicSpot does not take links; asked it to show");
            }
            return true;
        }
    }
...
mod bus_tests {
    ...
    fn a_link_reaches_the_running_instance_over_the_bus() {
        ...
        // shared name, so a MagicSpot already running is left alone
...
mod tests {
    ...
    fn parses_every_control_verb() {
        // #given / #when / #then
        assert_eq!(command("magicspot:show\n"), Some(ControlCommand::Show));
        assert_eq!(
            command("magicspot:playpause"),
            Some(ControlCommand::PlayPause)
        );
        assert_eq!(command("magicspot:play"), Some(ControlCommand::Play));
        assert_eq!(command("magicspot:pause"), Some(ControlCommand::Pause));
        assert_eq!(command("magicspot:next"), Some(ControlCommand::Next));
        assert_eq!(
            command("magicspot:previous"),
            Some(ControlCommand::Previous)
        );
        assert_eq!(
            command("magicspot:seek-by -10000"),
            Some(ControlCommand::SeekBy(-10_000))
        );
        assert_eq!(
            command("magicspot:volume-by +5"),
            Some(ControlCommand::VolumeBy(5))
        );
        assert_eq!(
            command("magicspot:volume-set 40"),
            Some(ControlCommand::SetVolume(40))
        );
        assert_eq!(command("magicspot:mute"), Some(ControlCommand::ToggleMute));
        assert_eq!(
            command("magicspot:shuffle"),
            Some(ControlCommand::ToggleShuffle)
        );
        assert_eq!(
            command("magicspot:repeat"),
            Some(ControlCommand::CycleRepeat)
        );
        assert_eq!(
            command("magicspot:shuffle-set on"),
            Some(ControlCommand::SetShuffle(true))
        );
        assert_eq!(
            command("magicspot:shuffle-set off"),
            Some(ControlCommand::SetShuffle(false))
        );
        assert_eq!(
            command("magicspot:repeat-set track"),
            Some(ControlCommand::SetRepeat(RepeatMode::Track))
        );
        assert_eq!(
            command("magicspot:repeat-set context"),
            Some(ControlCommand::SetRepeat(RepeatMode::Context))
        );
        assert_eq!(
            command("magicspot:repeat-set off"),
            Some(ControlCommand::SetRepeat(RepeatMode::Off))
        );
        assert_eq!(
            command("magicspot:seek-to 90000"),
            Some(ControlCommand::SeekTo(90_000))
        );
        assert_eq!(
            command("magicspot:save-toggle"),
            Some(ControlCommand::ToggleSaved)
        );
        assert_eq!(
            command("magicspot:play-uri spotify:playlist:37i9dQZF1DXcBWIGoYBM5M"),
            Some(ControlCommand::PlayUri(
                "spotify:playlist:37i9dQZF1DXcBWIGoYBM5M".to_owned()
            ))
        );
        assert_eq!(
            command("magicspot:transfer a1b2c3d4e5"),
        ...
        assert_eq!(
            command(
                "magicspot:open-link https://open.spotify.com/album/1DFixLWuPkv3KT3TnV35m3?si=x"
            ),
        ...
        assert!(matches!(
            parse("magicspot:nowplaying"),
            Some(Request::NowPlaying)
        ));
        assert!(matches!(parse("magicspot:devices"), Some(Request::Devices)));
    }

    #[test]
    fn rejects_lines_that_are_not_ours() {
        assert!(parse("GET / HTTP/1.1").is_none());
        assert!(parse("magicspot:frobnicate").is_none());
        assert!(parse("magicspot:seek-by soon").is_none());
        assert!(parse("magicspot:volume-set 999").is_none());
        assert!(parse("magicspot:next please").is_none());
    ...
    fn refuses_arguments_that_are_not_shaped_like_spotifys_own() {
        // #given / #when / #then
        assert!(command("magicspot:play-uri http://example.com/pwn").is_none());
        assert!(command("magicspot:play-uri spotify:track:a b").is_none());
        assert!(command("magicspot:play-uri ../../etc/passwd").is_none());
        assert!(command("magicspot:play-uri").is_none());
        assert!(command(&format!("magicspot:play-uri spotify:{}", "x".repeat(200))).is_none());
        assert!(command("magicspot:transfer ../secrets").is_none());
        assert!(command("magicspot:transfer").is_none());
        assert!(command("magicspot:open-link https://example.com/track/x").is_none());
        assert!(command("magicspot:open-link spotify:user:someone").is_none());
        assert!(command("magicspot:open-link").is_none());
        // A word that is not one of the three is refused rather than read
        // as `off`, which is what `RepeatMode::from_api` would have done.
        assert!(command("magicspot:repeat-set sometimes").is_none());
        assert!(command("magicspot:shuffle-set maybe").is_none());
        assert!(command("magicspot:seek-to -1").is_none());
    }
