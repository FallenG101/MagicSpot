...
fn success_page() -> String {
    page(
        "Signed in to MagicSpot",
        "You're signed in",
        "You can close this tab and go back to MagicSpot.",
        "#1ed760",
    )
}
...
fn failure_page(reason: &str) -> String {
    page(
        ...
        &format!("{reason}. Return to MagicSpot and try again."),
        "#f5717f",
    )
}
...
mod tests {
    ...
    fn stored_token_round_trips_and_tracks_expiry() {
        ...
        let dir = std::env::temp_dir().join(format!("magicspot-token-{}", std::process::id()));
