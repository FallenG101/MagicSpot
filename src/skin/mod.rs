...
//! [`layout`] defines window positions. Missing files fall back to MagicSpot's
...
pub enum SkinError {
    ...
    #[error("this is a modern Winamp skin, which MagicSpot cannot draw; it needs a classic one")]
    ModernSkin,
...
impl Skin {
    ...
    /// The skin MagicSpot ships, drawn for it and packed as a `.wsz` like
...
const BUILTIN_ARCHIVE: &[u8] = include_bytes!("../../assets/skins/magicspot.wsz");

static BUILTIN: LazyLock<Arc<Skin>> = LazyLock::new(|| {
    Arc::new(Skin::from_archive("MagicSpot", BUILTIN_ARCHIVE).expect("the built-in skin reads"))
});
...
mod tests {
    ...
    fn a_folder_of_bitmaps_is_a_skin_too() {
        let dir = std::env::temp_dir().join(format!("magicspot-skin-{}", std::process::id()));
    ...
    /// Loads every skin in `$MagicSpot_SKIN_SAMPLES`, when set, to check
    /// the reader against real files without shipping any.
    #[test]
    fn sample_skins_load() {
        let Ok(dir) = std::env::var("MagicSpot_SKIN_SAMPLES") else {
            return;
        };
        let mut seen = 0;
