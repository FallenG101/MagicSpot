...
fn main() -> anyhow::Result<()> {
    ...
    let receivers = magicspot::zeroconf::discover(Duration::from_secs(4))?;
    ...
    for receiver in &receivers {
        ...
        match magicspot::zeroconf::get_info(&http, receiver) {
            Ok(info) => println!(
                "  [{} {} | token={} | active_user={:?}]",
    ...
    let credentials = magicspot::zeroconf::Credentials::load(&dirs.credentials_dir())?;
    println!("\nhanding the account to {} ...", receiver.name);
    let info = magicspot::zeroconf::get_info(&http, receiver)?;
    match magicspot::zeroconf::add_user(&http, receiver, &info, &credentials, "MagicSpot") {
        Ok(()) => println!("  accepted"),
        Err(error) => {
...
fn devices() -> anyhow::Result<Vec<String>> {
    ...
    let token: serde_json::Value = serde_json::from_str(&std::fs::read_to_string(format!(
        "{home}/.local/state/magicspot/web_api_token.json"
    ))?)?;
